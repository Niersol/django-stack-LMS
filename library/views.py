...
def save_user(request):
    ...
    if not request.user.is_authenticated:
        resp['msg'] = "User is not authenticated"
...
def save_sub_category(request):
    ...
    if request.method == 'POST':
        ...
        resp['msg'] = "There is no data sent on the request"
...
def delete_sub_category(request,pk = None):
    ...
    if pk is None:
        ...
        try:
            ...
            messages.success(request,"Sub Category has been deleted successfuly.")
            resp['status'] = 'success'
        except:
...
def save_book(request):
    ...
    if request.method == 'POST':
        ...
        if not post['id'] == '':
            ...
            form = forms.SaveBook(post,instance=book)
        else:
            form = forms.SaveBook(post)
        ...
        if form.is_valid():
            ...
            if post['id'] == '':
                ...
                messages.success(request,"Book has been upddated successfully.")
            resp['status'] = 'success'
        else:
            for field in form:
                for error in field.errors:
...
def manage_book(request,pk = None):
    ...
    context['page_title'] = 'Managa Book'
    if pk is None:
        context['book'] = {}
    else:
        context['book'] = models.Books.objects.get(id=pk)
...
def delete_student(request,pk = None):
    ...
    if pk is None:
        ...
        try:
            models.Students.objects.filter(id =pk).update(delete_flag = 1)
...
def borrows(request):
    ...
    context['borrows'] = models.Borrow.objects.filter(delete_flag = 0).all()
