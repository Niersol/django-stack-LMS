...
class SaveBook(forms.ModelForm):
    ...
    def clean_isbn(self):
        ...
        existing_books = models.Books.objects.filter(isbn = isbn,delete_flag=0).exclude(id = id)
        if existing_books.exists():
            raise forms.ValidationError("This isbn is already registered with another book")
        
        return isbn 
class SaveStudent(forms.ModelForm):
    ag_number = forms.CharField(max_length=250)
    ...
    class Meta:
        ...
        fields = ('ag_number', 'first_name', 'middle_name', 'last_name', 'gender', 'contact', 'email', 'address', 'department', 'course', 'status', )

    def clean_code(self):
        id = int(self.data['id']) if (self.data['id']).isnumeric() else 0
        ag_number = self.cleaned_data['ag_number']
        existing_students = models.Students.objects.filter(ag_number= ag_number, delete_flag=0).exclude(id=id)

        if existing_students.exists():
            raise forms.ValidationError("Student School Id already exists in the database.")
...
class SaveBorrow(forms.ModelForm):
    ...
    def clean_student(self):
        ...
        if not student_id.isnumeric() or int(student_id)<=0:
            raise forms.ValidationError("Invalid Student")
        student = get_object_or_404(models.Students,id=int(student_id))
        return student
